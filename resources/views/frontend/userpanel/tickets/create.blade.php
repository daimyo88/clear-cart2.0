						...
						{{-- <div class="form-group">
							...
							<select class="form-control{{ $errors->has('ticket_category') ? ' is-invalid' : '' }}"
								...
								<option value="{{ $userTicketCategory->id }}"
									@if(old('ticket_category')==$userTicketCategory->id) selected @endif>{{
									\App\Classes\LangHelper::getValue(app()->getLocale(), 'ticket-category', null,
									$userTicketCategory->id) ?? $userTicketCategory->name }}</option>
								@endforeach
							</select>
